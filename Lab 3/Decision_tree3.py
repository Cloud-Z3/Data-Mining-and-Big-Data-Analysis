...
def classify(inputTree,featLabels,testVec):
    key=inputTree.keys()
    listkey=list(key)
    nextSet=inputTree[listkey[0]]
    flag=False
    for key in nextSet.keys():
        featIndex=featLabels.index(listkey[0])+1
        if testVec[featIndex]==key:

            if type(nextSet[key]).__name__=='dict':
                classLabel=classify(nextSet[key],featLabels,testVec)
            else:
                classLabel=nextSet[key]
